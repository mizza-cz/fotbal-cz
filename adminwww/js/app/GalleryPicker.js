function GalleryPicker(searchFieldId, pickerJsObjectName) {
  this.timeoutTimer = null;

  this.position = 1;

  // z GalleryPicker.latte
  this.refreshLink = refreshGalleryLink;
  ...
  this.Init = function () {
    ...
    // Kliknutí na galerii v seznamu nalezených
    $("#gallery-picker-wrapper").on(
      "click",
      "#" + this.pickerJsObjectName + "-listWrapper ul li a",
      $.proxy(function (event) {
        var gallery_id = parseInt($(event.target).attr("rel"));
        this.SelectGalleryAndClose(gallery_id);
        return false;
      }, this)
    );
  };

  this.SelectGalleryAndClose = function (gallery_id) {
    postEdit.InsertGalleryTag(gallery_id);
    $("#gallery-picker-wrapper").modal("hide");
  };
  ...
  this.MoveSelectionDown = function () {
    var current = $(".gallery-picker ul li.selected");
  ...
  this.ChooseSelectedGallery = function () {
    var current = $(".gallery-picker ul li.selected a");
    if (current.length > 0) {
      var gallery_id = parseInt(current.attr("rel"));
      this.SelectGalleryAndClose(gallery_id);
    }
  };
  ...
  this.OnKeydown = function (e) {
    ...
    if (e.keyCode == 38 || e.keyCode == 40 || e.keyCode == 13) {
      ...
      if (e.keyCode == 13) this.ChooseSelectedGallery();
      e.cancelBubble = true;
      return false;
    }
    return true;
  ...
  this.UpdateGalleryList = function () {
...
var galleryPickerObject = new GalleryPicker(searchFieldGalleryId, "galleryPickerObject"); // z GalleryPicker.latte

$(function () {
  galleryPickerObject.Init();
});
